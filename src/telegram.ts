...
export const formatNewOrderAlert = (order: any, userName: string) => {
  return `
...
<b>Items:</b>
${order.items.map((i: any) => `- ${i.productName || 'Instrument'} (x${i.quantity})`).join('\n')}

<pre>View in Admin Dashboard</pre>
  `.trim();
};

export const formatLowStockAlert = (product: any) => {
  return `
