    ...
    asyncHandler(async (req, res) => {
      ...
      const formatted = orders.map((o) => ({
        ...
        items_summary: o.items
          .map((i) => `${i.product?.name || "Instrument"} (x${i.quantity})`)
          .join(", "),
      }));
      res.json(formatted);
    }),
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        await sendTelegramAlert(
          formatNewOrderAlert(orderWithNames, req.user.name),
        );
      } catch (notifErr) {
  ...
  app.use((err, req, res, next) => {
    if (err instanceof multer.MulterError) {
      if (err.code === "LIMIT_FILE_SIZE") {
        return res
          .status(400)
          .json({ error: "File is too large. Max limit is 10MB." });
      }
...
process.on("unhandledRejection", (reason) => {
  console.error(
    ...
    reason instanceof Error ? reason : JSON.stringify(reason),
  );
});
